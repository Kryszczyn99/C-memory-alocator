...
#define PAGE_SIZE       4096    // D³ugoœæ strony w bajtach
#define PAGE_FENCE      1       // Liczba stron na jeden p³otek
#define PAGES_AVAILABLE 16384   // Liczba stron dostêpnych dla sterty
...
void __attribute__((constructor)) memory_init(void)
{
    ...
    /*
     * Architektura przestrzeni dynamicznej dla sterty, z p³otkami pamiêci:
     * 
     *  |<-   PAGES_AVAILABLE            ->|
     * ......................................
    ...
    //
    // Inicjuj strukturê opisuj¹c¹ pamiêæ procesu (symulacjê tej struktury)
...
void __attribute__((destructor)) memory_check(void)
{
    ...
    printf("\n### Stan p³otków przestrzeni sterty:\n");
    printf("    P³otek pocz¹tku: [%s]\n", first == 0 ? "poprawny" : "USZKODZONY");
    printf("    P³otek koñca...: [%s]\n", last == 0 ? "poprawny" : "USZKODZONY");

    printf("### Podsumowanie: \n");
        printf("    Ca³kowita przestrzeni pamiêci....: %lu bajtów\n", mm.start_mmap - mm.start_brk);
        printf("    Pamiêæ zarezerwowana przez sbrk(): %lu bajtów\n", mm.brk - mm.start_brk);
    
    //if (first || last) {
        printf("Naciœnij ENTER...");
        fgetc(stdin);
    //}
}
